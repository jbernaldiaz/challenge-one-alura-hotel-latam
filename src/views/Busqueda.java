	...
	private void LlenarTablaReservas() {
		...
		try {
			for (Reservas reservas : reserva) {
				modelo.addRow(new Object[] { reservas.getId(), reservas.getFecha_entrada(), reservas.getFecha_salida(), reservas.getValor(), reservas.getFormaPago() });
			}
		} catch (Exception e) {
			throw e;
		}
	...
	private void LlenarTablaHuespedes() {
		...
		try {
			for (Huespedes huespedes : huesped) {
				modeloHuesped.addRow(new Object[] { huespedes.getId(), huespedes.getNombre(), huespedes.getApellido(), huespedes.getFecha_nacimiento(), huespedes.getNacionalidad(), huespedes.getTelefono(), huespedes.getIdReservas() });
			}
		} catch (Exception e) {
			throw e;
		}
	...
	private void ActualizarReservas() {
		Optional.ofNullable(modelo.getValueAt(tbReservas.getSelectedRow(), tbReservas.getSelectedColumn()))
				.ifPresentOrElse(fila -> {
					...
					String fechaE = String.valueOf(modelo.getValueAt(tbReservas.getSelectedRow(), 1).toString());
					String fechaS = String.valueOf(modelo.getValueAt(tbReservas.getSelectedRow(), 2).toString());
	...
	private void ActualizarHuesped() {
		Optional.ofNullable(modeloHuesped.getValueAt(tbHuespedes.getSelectedRow(), tbHuespedes.getSelectedColumn()))
				.ifPresentOrElse(filaHuesped -> {
					...
					String fechaN = String.valueOf(modeloHuesped.getValueAt(tbHuespedes.getSelectedRow(), 3).toString());
